...
% Tout d'abord, concernant la position de l'effecteur.
% Les effets générés lors du babillage sont effectués à partir de différentes faces de l'objet.
% Ainsi, lorsque le robot tente de pousser l'objet vers la zone cible, en fonction de la position de cette zone à atteindre, les effets seront différents ainsi que les faces/points de contacts d'où initier les effets.
% \`A chaque effet est associé un point de contact dans le repère de l'objet.
% Cela nous a amené à récupérer les coordonnées du point de contact pour savoir d'où initier l'effet.
% Ces coordonnées sont exprimées dans le repère lié à l'objet et sont corrélées à l'effet enregistré.
% La figure \ref{fig:contact_position} présente ce cas particulier.
